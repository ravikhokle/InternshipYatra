...
const ViewDetails = () => {
  ...
  useEffect(() => {
    const getInternshipDetails = async () => {
      try {
        const response = await axiosInstance.get('/posts/viewdetails', {
          params: { id },
        });
  ...
  const handleApplyClick = async (postId) => {
    ...
    try {
      ...
      if (response.status === 200) {
        handleSuccess(
          response?.data.message || "You have successfully applied for the internship!"
        );
      }
    } catch (error) {
            ...
            Posts.map((post, index) => (
              ...
              >
                ...
                <div className="text-base text-gray-700 space-y-4">
                  <p><span className="font-medium">Stipend:</span> &#8377;{post.stipend}</p>
                  <p><span className="font-medium">Skills Required:</span>{" "}{post.skills || "Not specified"}</p>
                  <p><span className="font-medium">Duration:</span> {post.duration}</p>
                  <p>
